...
use App\Services\Winstyle\Helper;
...
class ItemControllerDev extends Controller
{
    public string $link;
    public int $product_id;
    public string $ses_id;
    public array $page_error = [];
    public array $nodes_error = [];
...
/*        try {
            Artisan::call('winstyle:item', ['url' => $this->link]);
            return Artisan::output();
        } catch (\Exception $e) {
            dd($e);
        }*/

        return file_get_contents('/home/murin/_dev/_Denis/vinobaza/vinobaza-front/htmlItem.txt');
    }
...
//            $imgOriginal->response($ext);
            ...
            if(\Storage::disk('public')->put('/uploads/product/orig/' . $filename,  $response) && !is_null($file)){
               $f = File::updateOrCreate(['product_id' => $this->product_id],[
                    'name' => $filename,
                    'type' => 'image',
                    'from' => 'winestyle',
                ]);

                $imgMiddle = $manager->make(public_path('/storage/uploads/product/orig/').$filename);
                $imgMiddle->resize(null, '452', function ($constraint) {
                    $constraint->aspectRatio();
                })->response($ext);

                $imgThumb = $manager->make(public_path('/storage/uploads/product/orig/').$filename);
...
// product
    ...
    public function prepare(){
        ...
        if($this->item['grape']){
            $arGrapeIDs = [];
            $arGrapeIPercentage = [];
            foreach ($this->item['grape'] as $grapeType){
